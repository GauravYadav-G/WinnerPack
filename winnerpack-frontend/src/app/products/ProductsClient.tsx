  ...
  return (
    <div className="min-h-screen bg-white text-[var(--color-text)]">
      ...
      <PageWrapper>
        <PageHeader
          title="Our Products"
          eyebrow="Packaging That Performs From Line to Delivery."
          ...
          <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
              <div className="max-w-2xl">
                <p className="font-mono text-xs font-bold tracking-[0.16em] text-[var(--color-amber-dark)]">Browse by Material</p>
                  ...
                  >
                    <Link href={`/product-category/${category.id}`} className="group flex h-full flex-col overflow-hidden rounded-2xl sm:rounded-3xl border border-slate-200/90 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-[var(--color-amber)]/60 hover:shadow-lg active:scale-[0.99] sm:active:scale-100 shadow-sm">
                      ...
                      <div className="flex flex-1 flex-col justify-between p-4 sm:p-5">
                        <div>
                          <p className="font-mono text-xs font-bold uppercase tracking-[0.12em] text-[var(--color-amber-dark)]">{category.items.length} product formats</p>
                          ...
                          <p className="hidden sm:block mt-1.5 text-sm leading-relaxed text-[var(--color-mute)]">{category.blurb}</p>
                        </div>
                        <div className="mt-4 flex items-center justify-between border-t border-[var(--color-line)] pt-3 text-sm font-bold text-[var(--color-ink)]"><span>Explore range</span><ArrowRight className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[var(--color-amber-dark)] transition-transform group-hover:translate-x-1" /></div>
                      </div>
                    </Link>
                  </motion.div>
                );
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="rounded-2xl sm:rounded-3xl bg-[var(--color-blue-deep)] px-5 py-6 sm:px-10 sm:py-10 text-white shadow-xl">
              <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                  <p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-[var(--color-amber)]">Custom requirement?</p>
                  <h2 className="mt-1.5 font-display text-xl sm:text-3xl font-extrabold">Get a material recommendation from our team.</h2>
                  <p className="mt-2 sm:mt-3 max-w-3xl text-sm leading-relaxed text-white/75">Share your product, packaging line, and dispatch goal. We’ll help you identify the practical material and specification.</p>
                  <ul className="mt-4 sm:mt-5 flex flex-wrap gap-x-4 sm:gap-x-5 gap-y-2 text-sm font-semibold text-white/90">
                ...
                <Link href="/contact" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--color-amber)] px-5 py-3.5 text-sm font-extrabold text-[var(--color-ink)] transition hover:bg-[var(--color-amber-2)] active:scale-95 shadow-md min-h-[44px]">
