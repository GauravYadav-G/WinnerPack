...
export default function AdminBlogsPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await Promise.all([
          ...
          slug ? triggerRevalidate(`/blog/${slug}`) : Promise.resolve()
        ]);
  ...
  const handleDeleteArticle = async (id: string) => {
    ...
    try {
      ...
      const slug = article ? article.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)+/g, "") : "";

      const res = await apiFetch(`/api/articles?id=${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        // Trigger on-demand revalidation
