      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
        ...
        <div className="grid gap-px overflow-hidden rounded-2xl border border-[var(--color-line)] bg-[var(--color-line)] grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 shadow-xl" data-reveal>
          {uspsList.map((u, i) => {
            ...
            const bgImage = u.bgImage || defaultUsps[i]?.bgImage;
            return (
              <div
                key={u.title}
                className="group relative overflow-hidden bg-white p-5 sm:p-7 md:p-9 lg:p-10 transition-all duration-500 hover:bg-slate-950 min-h-[220px]"
                data-hover
              >
                {/* Hover-Reveal Contextual Background Image with reduced overlay opacity for clear visibility */}
                {bgImage && (
                  <div
                  ...
                  >
                    ...
                    />
                    {/* Balanced dark gradient overlay with subtle blur so text pops over the background image */}
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/40 to-slate-950/20 backdrop-blur-[2px]" />
                  </div>
                )}
                ...
                <h3 className="relative font-display text-sm sm:text-base md:text-xl lg:text-2xl font-bold leading-tight text-[var(--color-ink)] transition-colors group-hover:text-white group-hover:drop-shadow-md">
                  {u.title}
                </h3>
                <p className="relative mt-2 md:mt-3 text-sm leading-relaxed text-[var(--color-mute)] transition-colors group-hover:text-white/90 group-hover:drop-shadow-sm line-clamp-4">
                  {u.text}
                </p>

                <div className="relative mt-4 md:mt-7 flex items-center gap-1.5 md:gap-2 font-mono text-xs font-bold tracking-widest text-[var(--color-amber)] opacity-0 transition-opacity duration-500 group-hover:opacity-100 drop-shadow-sm">
                  Verified Spec
